...
impl PrimitiveType {
  ...
  const OFFSET_MASK: u64 = 0xFF_0000_0000;
  const OFFSET_BIT_OFFSET: u64 = 32;
}
...
impl PrimitiveType {
  ...
  pub const Descriminant: PrimitiveType = PrimitiveType((PrimitiveSubType::Descriminant as u64) << PrimitiveType::SUBTYPE_BIT_OFFSET);
...
impl PrimitiveType {
  ...
  pub fn bit_offset(&self) -> u64 {
    ((self.0 & PrimitiveType::OFFSET_MASK) >> PrimitiveType::OFFSET_BIT_OFFSET).max(0)
  }
...
impl std::ops::BitOr<Offset> for PrimitiveType {
  type Output = PrimitiveType;

  fn bitor(self, rhs: Offset) -> Self::Output {
    Self((self.0 & !Self::OFFSET_MASK) | (Self::OFFSET_MASK & ((rhs.0 as u64) << Self::OFFSET_BIT_OFFSET)))
  }
}
...
impl Display for PrimitiveType {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    let offset = match self.bit_offset() {
      0 => String::default(),
...
fn test_primitive_type() {
  ...
  assert_eq!(format!("{}", PrimitiveType::Signed | Offset(23) | BitSize::b8), "i8<<23");


  assert_eq!(format!("{}", PrimitiveType::Descriminant | BitSize::b(13)), "dsc13");
