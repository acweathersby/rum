...
fn allocate_from_heap(ctx: &mut RuntimeSystem, heap_name: CMPLXId, size: u64) -> *mut u8 {
  if let Some(target_heap) = ctx.heaps.get(&heap_name).or_else(|| Default::default()) {
    ...
    let sig = Signature::new(&[(Default::default(), heap_type.clone()), (Default::default(), ty_u64), (Default::default(), allocator_i.clone())], &[(Default::default(), ty_addr)]).hash();

    if let Some(allocate_method_id) =
      ctx.db.interface_instances.get(&allocator_i).expect("Could not find allocate interface instances").get(heap_type).expect("Could not find interface for heap").get(&sig)
    {
...
#[inline]
fn interprete_binary_args(super_node: &RootNode, op: OpId, operands: &[OpId; 3], scratch: &mut Vec<(Value, u32)>, ctx: &mut RuntimeSystem, scope_data: ScopeData) -> (Value, Value) {
...
#[inline]
fn interprete_binary_cmp_args(super_node: &RootNode, op: OpId, operands: &[OpId; 3], scratch: &mut Vec<(Value, u32)>, ctx: &mut RuntimeSystem, scope_data: ScopeData) -> (Value, Value) {
