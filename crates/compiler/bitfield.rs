...
impl BitFieldArena {
  ...
  /// Does a bitwise or between source and destination and places the result in destination
  pub fn or(&mut self, dst: usize, src: usize) -> bool {
    let left = dst * self.row_ele_size;
    let right = src * self.row_ele_size;
    let len = self.row_ele_size;
    let slice = self.slice_mut();
    let mut diff = false;
  ...
  // Copies src to dst. Returns true if any bits are different between the to
  pub fn mov(&mut self, dst: usize, src: usize) -> bool {
    let left = dst * self.row_ele_size;
    let right = src * self.row_ele_size;
    let len = self.row_ele_size;
    let slice = self.slice_mut();
  ...
  pub fn iter_set_indices_of_row<'a>(&'a self, row: usize) -> impl Iterator<Item = usize> + 'a {
